...
let connectionString = `mongodb+srv://${process.env.MONGOUSERNAME}:${process.env.MONGOPASSWORD}@mongosetupcluster.li7admh.mongodb.net/FoodDatabase?retryWrites=true&w=majority`;
...
app.post('/create_fruit', async (req, res) =>{
    ...
    const {nameString: fruitName, colorString: color, ageNumber: age, readyBool: readyToEat} = req.body;
...
app.post('/create_veggie', async (req,res) => {
    const {nameString: veggieName, colorString: color, ageNumber: age, readyBool: readyToEat} = req.body;
...
app.get('/veggie/:veggieName', async (req,res) => {
    ...
    let response = await MyVeggie.find({veggieName: veggie})
    res.json(response)
})
